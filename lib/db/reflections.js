...
async function runQfSyncCreate(ref, payload) {
  ...
  if (current.exists && current.data()?.qfNoteId) {
    await ref.update({ syncStatus: "synced", updatedAt: admin.firestore.Timestamp.now() });
    return { qfNoteId: current.data().qfNoteId, syncStatus: "synced" };
  }
  ...
  if (!accessToken) {
    ...
    return { qfNoteId: null, syncStatus: "synced" };
  }
  ...
  const syncStatus = qfNoteId ? "synced" : "failed";
  await ref.update({
...
export async function updateReflection(id, appUserId, updates = {}) {
  ...
  void (async () => {
    ...
    if (qfNoteId) {
      ...
      if (created) {
        await ref.update({ qfNoteId: created, syncStatus: "synced", updatedAt: admin.firestore.Timestamp.now() });
        return;
      }
      syncStatus = "failed";
    }
    await ref.update({ syncStatus, updatedAt: admin.firestore.Timestamp.now() });
  })().catch(() => {});
