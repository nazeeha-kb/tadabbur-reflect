...
export function resolveOAuthRedirectUri(requestUrl) {
  ...
  const hasQF_REDIRECT_URI = Boolean(process.env.QF_REDIRECT_URI?.trim());
  const hasQF_OAUTH_REDIRECT_URI = Boolean(process.env.QF_OAUTH_REDIRECT_URI?.trim());
  ...
  if (fromEnv) {
    ...
    resolved = `${new URL(requestUrl).origin}/api/auth/callback`;
    source = "request.url";
  }

  console.log("[resolveOAuthRedirectUri]:", {
    hasQF_REDIRECT_URI,
    hasQF_OAUTH_REDIRECT_URI,
...
export async function buildAuthorizationUrl({ redirectUri, scopes }) {
  console.log("[buildAuthorizationUrl] starting...");

  let config;
  try {
    config = getQfOAuthConfig();
    console.log("[buildAuthorizationUrl] got OAuth config");
  } catch (err) {
    console.error("[buildAuthorizationUrl] getQfOAuthConfig() failed:", err.message);
    throw err;
  }
  ...
  const scopeList =
    scopes && scopes.length > 0 ? scopes : ["openid", "offline_access", "user", "collection", "streak", "note"];
  ...
  try {
    ...
    console.log("[buildAuthorizationUrl] PKCE cookie set successfully");
  } catch (err) {
    console.error("[buildAuthorizationUrl] cookie.set() failed:", err.message);
    throw err;
  }

  const auth = baseUrl.replace(/\/$/, "");
...
function issuerMatches(iss, baseUrl) {
  if (!iss) return true;
  const normalizedIss = String(iss).replace(/\/$/, "");
  const normalizedAuth = baseUrl.replace(/\/$/, "");
...
export function validateIdTokenClaims({ idToken, expectedNonce }) {
  ...
  const { clientId, baseUrl } = getQfOAuthConfig();
  ...
  if (!issuerMatches(claims.iss, baseUrl)) throw new Error("invalid_id_token_issuer");
...
export async function exchangeAuthorizationCode({ code, redirectUri, codeVerifier, isConfidential }) {
  const { baseUrl, clientId, clientSecret } = getQfOAuthConfig();
...
export async function refreshAccessToken({ refreshToken, isConfidential }) {
  ...
  if (!response.ok) {
    ...
    console.error("QF REFRESH FAILED RAW:", {
