...
function normalizeQfStreak(data) {
  ...
  // handle paginated empty response
...
export async function getStreakWithFallback({ appUserId } = {}) {
  ...
  if (client) {
    try {
      const response = await client.fetch("/streaks?first=1");
      if (response.ok) {
        const json = await response.json();
        const normalized = normalizeQfStreak(json?.data?.streak ?? json?.data ?? json);

        if (normalized) {
          streakLog("streak.fetch.qf", {
            appUserId,
        ...
        console.log("RAW QF STREAK RESPONSE:", JSON.stringify(json, null, 2));
        streakLog("streak.fetch.qf_empty", { appUserId });
        return null;
      }
