...
function buildSdkServices({ baseUrl, apiBaseUrl }) {
  const apiRoot = apiBaseUrl.replace(/\/$/, "");
  const authRoot = baseUrl.replace(/\/$/, "");
...
export function getQfSdkClient() {
  if (!clientInstance) {
    ...
    const { clientId, clientSecret, baseUrl, apiBaseUrl, env } = config;
    if (!clientSecret) {
      throw new QuranSearchError(
        SearchErrorCode.CONFIG,
    ...
    clientInstance = createServerClient({
      clientId,
      clientSecret,
      services: buildSdkServices({ baseUrl, apiBaseUrl }),
    });

    if (process.env.NODE_ENV === "development" || process.env.SEARCH_DEBUG === "1") {
      console.info("[search:sdk.client]", { env, baseUrl, apiBaseUrl });
    }
  }
