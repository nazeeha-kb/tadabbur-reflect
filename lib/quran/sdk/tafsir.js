...
const PRIMARY_SOURCE_ID = "ibn-kathir";
...
/**
 * Ordered SDK resource ids: requested → ibn-kathir → tazkirul-quran → any English → any.
 */
...
export async function fetchTafsirForVerseWithFallback(verseKey, requestedSourceId) {
  ...
  const flightKey = `${verseKey}:${requestedSourceId || "default"}`;
  if (tafseerInFlight.has(flightKey)) {
    return tafseerInFlight.get(flightKey);
  }

  const task = fetchTafsirForVerseWithFallbackInner(verseKey, requestedSourceId);
  tafseerInFlight.set(flightKey, task);
  try {
    return await task;
  } finally {
    tafseerInFlight.delete(flightKey);
  }
}

async function fetchTafsirForVerseWithFallbackInner(verseKey, requestedSourceId) {
  try {
