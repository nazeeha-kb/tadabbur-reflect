...
async function searchVersesPublicOrEmpty(searchTerms) {
  try {
    return await searchVersesPublic(searchTerms);
  } catch {
    return [];
  }
}

async function searchVersesFoundationOrEmpty(searchTerms) {
  try {
    return await searchVersesFoundation(searchTerms);
  } catch {
    return [];
  }
}
 ...
 */
async function searchVersesMerged(queries) {
  ...
  const pushBatch = (batch) => {
    ...
    for (const a of batch) {
      ...
      seen.add(key);
      merged.push(a);
      if (merged.length >= 5) return true;
    }
    return false;
  };
  ...
  if (isQuranFoundationConfigured()) {
    ...
    for (const q of list) {
      ...
      const batch = usePublic ? await searchVersesPublicOrEmpty(q) : await searchVersesFoundationOrEmpty(q);
      if (pushBatch(batch)) break;
    }
  } else {
    for (const q of list) {
      if (merged.length >= 5) break;
      const batch = await searchVersesPublicOrEmpty(q);
      if (pushBatch(batch)) break;
    }
  }
...
export async function getAyahsByEmotion(rawQuery) {
  ...
  const enriched = await enrichAyahsWithTafsir(ayahs);
